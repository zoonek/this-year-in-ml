...
\item The Markdown produce by ChatGPT was (mostly) correct, but pandoc completely mangled the math.
...
causal graph. The authors show that the Hessian entries $\partial^2 / (\partial x_i\,\partial x_j)$
log p(x) vanish exactly for non-adjacent nodes, and exploit this to
...
\item
  Predictability $\nu =
  \frac{\mathrm{Var}(\hat p_t)}{\mathrm{Var}(p_t)}$).
\end{itemize}
...
regimes. Nystrup et al.~and Aydinḥan et al.~apply these to financial
