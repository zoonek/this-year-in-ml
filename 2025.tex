...
\begin{reference}
  \emph{\emph{\url{http://zoonek.free.fr/Ecrits/articles.pdf}}}
\end{reference}
...
this is the \d{null hypothesis}, $H_0$ -- a set of probability distributions,
for instance the set of all probability distributions on $\RR$ with mean zero.
We then compute a quantity, the \d{test statistic},
which has a known distribution(*)
if the data comes from one of the distributions in $H_0$.
If the test statistic is above its $1-\alpha$ quantile (the \d{critical value}),
for a predefined value of $\alpha$ (often 5\%),
we \d{reject} $H_0$ (we claim that there is something noticeable in the data),
...
\d{type I} error, incorrectly rejecting $H_0$;
\d{type II} error, not rejecting $H_0$ when we should have.

\subsection{p-values}

Instead of test statistics (each test has its own distribution,
so test statistics can be difficult to interpret), we can consider \d{p-values},
...
it is \emph{not} the probability that $H_0$ is true.
...
For instance, if  the null hypothesis is \d{composite}, i.e., if it contains several distributions,
...
be \d{stochastically greater} than Unif(0,1),
i.e., $P[p\leq\alpha]\leq\alpha$ for all distributions $P$ in $H_0$.
...
(FDR, \d{false discovery rate})
or at the probability of making one or more type I error in $N$ tests (FWER, \d{family-wise error rate}).
If the number of tests is large, it will be difficult to control the FWER.

Adjusting the p-values for the FWER is easy:
if the tests are independent, use \d{Šidák's adjustment},
\[ p \leftarrow 1 - (1-p)^N, \]
or its approximation, \d{Bonferroni's adjustment},
\[ p \leftarrow Np. \]

Those adjustments can be improved if you have the list of p-values (\d{Holm-Bonferroni}):
...
sort the p-values, from the smallest to the largest, and use the \d{Benjamini-Hochberg} (BH) adjustment,
\[ p_k \leftarrow (N/k) p_k \]
or, more precisely,
\[ p_k \leftarrow \min \bigl\{ N p_\ell / \ell , \ \ell \geq k \bigr\} \]
(for dependent tests, the \d{Benjamini-Yekutieli} (BHY) adjustment multiplies this by
$\sum_{k \leq N} 1/k$).
...
you can also use \emph{resampling} to control the FWER:
this will automatically account for dependencies between the tests.

\begin{reference}
  \emph{A reality check for data snooping}
  H.L. White (2000)
  \url{https://users.ssc.wisc.edu/~behansen/718/White2000.pdf}
\end{reference}

\begin{reference}
  \emph{Stepwise Multiple Testing as Formalized Data Snooping}
...
by estimating $P(H_0)$ (\d{q-values})
or the distribution of the p-values under $H_0$ and $H_1$ (\d{local FDR}).
...
\begin{reference}
  \emph{Local false discovery rates}
  B. Efron (2005)
...
We can proceed as follows (\d{group sequential design}).
...
for instance, we can choose them to be constant (\d{Pocock boundary})
or (better) to be proportional to $1/\sqrt t$ (\d{O'Brien-Fleming} --
early stopping requires stronger evidence).
It is also possible to adjust the p-values.

More generally, \d{alpha-spending} considers a spending function
$\alpha: [0,1]\to[0,\alpha]$,
$\alpha(0)=0$, $\alpha(1)=\alpha$, increasing,
and uses critical values $c_t$ such that
\[
  P\bigl[ Z_t\!>\!c_t, Z_1\!<\!c_1, \dots,Z_{t-1}\!<\!c_{t-1} \bigr] =
  \alpha\left( \dfrac tT \right)
  -
  \alpha\left( \dfrac{t-1}{T} \right).
\]
...
Instead of a sequence of p-values, you could build an \d{e-process}:
...
and a generalization of \d{Kelly's principle},
...
second, the tests focus on the \d{log-power}, rather than the power.
...
for tabular data,
in part because of their \emph{rotation invariance} --
trees are \emph{axis-aligned}, which often helps them identify structure in tabular data.
...
\d{Foundational} tabular data models have started to appear, in particular TabPFN
...
and synthetic data may present artefacts or (when based on resampling)
...
For instance, \d{recursive feature machines} (RFM)
fit a ridge regression in a warped space,
$x \mapsto Mx$,
where $M$ is the \d{average gradient outer product} (AGOP)
...
Kernel methods do not scale to large datasets, but we can use the \d{Nyström approximation}.
...
\begin{reference}
  \emph{TimeFM: A decoder-only foundation model for time-series forecasting}
  A. Das et al. (2023)
...
\begin{reference}
  \emph{Moirai: Unified Training of Universal Time Series Forecasting Transformers}
  G. Woo et al. (2024)
...
If you want to check if a given \d{feature} is present
in a network, and where, you can train a \d{probe} (a simple, shallow neural network)
...
Instead of features, one can also look for interpretable \d{circuits}:
...
Most neurons are \d{polysemantic}: they detect several things at the same time,
i.e., they encode a \d{superposition} of features.
To account for this, you can learn a \d{sparse auto-encoder} (SAE) on the layer of interest:
...
\d{JumpReLU} activation (zero out activations below some threshold),
\d{Matryoshka loss} (several loss terms, for the reconstruction from the first $k_1$, $k_2$, $k_3$, etc. dimensions).
...
\d{Transcoders} are very similar to SAEs:
while SAEs try to reconstruct a layer's activations from that layer's activations
after going through a wider but sparse layer,
transcoders try to reconstruct the output (of the whole network, or just a subgraph)
from the input after going through a wide but sparse layer.
...
\begin{reference}
  \emph{Sirus: Interpretable random forests via rule extraction}
  C. B\'enard et al. (2020)
  \url{https://arxiv.org/abs/2004.14841}
\end{reference}

\begin{reference}
  \emph{RIPE: Rule induction partitioning estimator: design of an interpretable prediction algorihm}
  V. Margot et al. (2028)
  \url{https://arxiv.org/abs/1807.04602}
\end{reference}

One can also \d{distill} a random forest into a decision tree (a student tree).
...
For linear models, this is sometimes called \emph{relevance} forecasting.
...
\d{CMA-ES} is a simple, population-based optimization algorithm:
...
Chain-of-Thought (CoT, spliting a problem into a linear sequence of steps)
...
you can use \d{RAG} (retrieval-augmented generation): put all your documents
...
thanks to the addition of a sine (or learned) \d{positional embedding} (PE) to the first layer.
There are many variants:
for instance, \d{RoPE} multiplies keys and queries with a sine embedding (which rotates them),
while \d{ALiBi} adds position-based penalties to the attention scores.
...
if you use \emph{hashed} $n$-gram embeddings (for $n\leq8$).
...
The most successful current attacks are \d{decomposition attacks}:
...
The \d{BERT} model is not dead.
It is an encoder-only model (used mainly for classification -- LLMs are generative models).
...
On some tasks, e.g., \d{open-ended NER} (named entity recognition), they can compete with LLMs.
...
Vowpal Wabbit's \d{hashing trick} to store the coefficients of the linear model you train with them.
...
then they learn to behave as an assistant, i.e., to answer user's queries, with \d{supervised fine-tuning} (SFT),
and they are finally \d{aligned} to human preferences (do no harm) with reinforcement learning (RL).

Instead of SFT+RL, DeepSeek uses SFT+RL+SFT+RL;
for the reinforcement learning part, it uses \d{GRPO}, i.e.,
...
\d{LoRA} (low-rank adaptation) is one of the main
\d{PEFT} (parameter-efficient fine-tuning) methods:
...
\d{QLoRA} uses quantized weights;
\d{ReLoRA} periodically merges the LoRA weights into the base model and resets them;
\d{DoRA} extends LoRA to continual learning by encouraging updates orthogonal to those of the previous task;
\d{GaLore} reduces the memory requirement of gradient based optimization by replacing the gradients with low-rank approximations.
...
Instead of an actual prompt (a sequence of tokens), it could be a \d{soft prompt}: a sequence of embeddings --
...
\d{adapters} (small feed-forward layers inserted between layers),
...
The training \d{curriculum} also matters: easier samples are more useful at the begining (pre-training),
...
Instead, average highly correlated features (to keep only one copy of them) and concatenate the others.
...
\begin{reference}
  \emph{Traditional and heavy-tailed regularization in neural network models}
...
is exploding, and they become difficult to track.
\d{AdamW} seems to be the most popular for the moment,
...
\d{Muon} is a first-order method (for models whose weights are matrices and not just vectors),
...
There have been some attempt to train \d{foundational} causal discovery models.
...
The GES algorithm is a \d{greedy search}: it is suboptimal.
It can be improved by \d{boosting}, i.e.,
by re-fitting the models after increasing the weights of samples with a bad fit.
...
using \d{dynamic programming} or, better, \d{A*}.
...
this is what the \d{CAM} algorithm does.
...
unobserved confounders), we have a \d{structural equation model} (SEM).
...
this is what \d{VAR-LiNGAM} and \d{DynoTEARS} do.
...
its \d{spectral radius} is zero, i.e., if its only eigenvalue is zero:
...
this is what the \d{DAG-GNN}, \d{GranDAG} and \d{cGAN} models do.
...
For instance, they implicitly assume that the true causal graph is \d{sparse}:
...
but if there are \d{deterministic relations}, everything becomes (conditionally) independent.
You should remove those variables from your data or account for them.
...
you can try to use \d{extreme observations} as proxies for interventions --
they can be viewed as \d{natural experiments}.
...
for instance, with \d{rank conditions}.
...
almost everywhere.
You may be able to get away with \d{proxies} for missing confounders, though.
...
If in doubt, you can perform a \d{sensitivity analysis}, i.e.,
answer the question
...
(S-, T-, X-, R-, DR-learners), stressing the need for \d{cross-fitting}.
...
MM (\d{Majorization-Minimization}), SCA (\d{sequential convex approximation}),
\d{ADMM} (alternating direction of multipliers), and convex-concave fractional programs.
...
The book also has clear chapters on \d{learning graphs from data},
and on \d{risk parity}.
...
$A$ and $C$ are correlated, even if they are independent given $C$.
...
This is what the \d{graphical lasso}:
find a matrix close to the inverse of the sample correlation matrix,
with a sparsity penalty.

\begin{reference}
  \emph{Statistical learning with sparsity, chapter 9}
  T. Hastie et al. (2015)
  \url{https://hastie.su.domains/StatLearnSparsity/}
\end{reference}

There are variants of this approach, which look for the \d{Laplacian matrix} of the graph instead.
...
\begin{reference}
  \emph{Portfolio optimization, chapter 5}
  D.P. Palomar (2025)
  \url{https://portfoliooptimizationbook.com/}
\end{reference}
...
we could try to \emph{rank} stocks
...
we could directly forecast \emph{portfolio weights}.

\begin{reference}
  \emph{Interpretable supervised portfolios}
  G. Chevalier et al. (2022)
  \url{https://ssrn.com/abstract=4230955}
\end{reference}
...
You can also obtain such latent representations from text (news, 10K, etc.) or \emph{holdings data}.
...
\d{Diffusion} models consider the progressive addition of noise to an image,
...
and diffusion models such as \d{DDPM} learn the reverse-time SDE.
...
this is what the \d{DDIM} diffusion model does. The probability distributions are the same,
...
For instance, \d{classifier-free guidance} (CFG) models train a single model
...
\d{consistency models} try to do it all at once, by training a model to transform an image
...
Instead, it is actually possible to look for an \d{optimal decision tree};
...
Other methods to build (nearly) optimal decision trees include \d{frequent itemset mining}
...
one can use \d{proper scoring rules},
such as the \d{continuous ranked probability score} (CRPS):
...
by reducing the number of nodes (\d{coarsening}, i.e., merging nodes)
or the number of edges (\d{sparsification}).
...
The \d{FastFood} parametrization approximates Gaussian matrices.
...
The \d{sliced Wasserstein distance} projects the data on (fixed, but random)
